 ...
 * Class checks the permissions for the User Location
 */

public class PermissionUtility {

    private String TAG = "EatMeLocationUtility";
    ...
    public PermissionUtility(Context context, PermissionResultCallback callback)
    {
        this.context=context;
        this.current_activity= (Activity) context;
            ...
            case 1:
                if(grantResults.length>0)
                {
                    ...
                    for (int i = 0; i < listPermissionsNeeded.size(); i++)
                    {
                        if (perms.get(listPermissionsNeeded.get(i)) != PackageManager.PERMISSION_GRANTED)
                        {
                            if(ActivityCompat.shouldShowRequestPermissionRationale(
                                    current_activity,listPermissionsNeeded.get(i)))
                                pending_permissions.add(listPermissionsNeeded.get(i));
                            else
                            {
                                Log.d(TAG,"Go to settings and enable permissions " );
                                    ...
                                    public void onClick(DialogInterface dialog, int which) {
                                        ...
                                        switch (which) {
                                            ...
                                            case DialogInterface.BUTTON_NEGATIVE:
                                                Log.d(TAG,"Permisson not fully given");
                                                if(permission_list.size()==pending_permissions.size())
                                                    permissionResultCallback.PermissionDenied(req_code);
                                                else
                                                    permissionResultCallback.PartialPermissionGranted(
                                                            req_code,pending_permissions);
                                                break;
                                        }
                    ...
                    else
                    {
                        ...
                        Log.d(TAG,"All permissions granted");
                        Log.d(TAG,"Proceed to next step");
